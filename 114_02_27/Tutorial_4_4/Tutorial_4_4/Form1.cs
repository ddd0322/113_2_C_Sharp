        ...
        private void calculatwbutton_Click(object sender, EventArgs e)
        {
            ...
            if (double.TryParse(distanceTextBox.Text, out distance))
            {
                if (double.TryParse(gasTextBox.Text, out gas))
                {
                    ...
                    averageLabel.Text = "平均油耗:" + average.ToString("f2") + "公里/公升"; // 修正變數名稱和語法錯誤
                    logListBox.Items.Add(average.ToString("f2") + "公里/公升"); // 修正變數名稱和語法錯誤
                }
                else
                {
                    MessageBox.Show("油耗請輸入數字");
                    gasTextBox.Focus(); // 將滑鼠游標聚焦到輸入框
            ...
            else
            {
                ...
                distanceTextBox.Focus(); // 將滑鼠游標聚焦到輸入框
